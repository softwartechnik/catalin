...
public class CatalinSwingGUI implements CatalinGUI {
  ...
  @Inject
  public CatalinSwingGUI(CatalinCoreConfig catalinCoreConfig, CatalinGUIController controller,
      PersonService personService, AirlineService airlineService, BookingService bookingService,
      AirportService airportService, EmployeeService employeeService, FlightService flightService) {
  ...
  private void initMocks() {
    ...
    Flight ffmMunich = flightService
        .createFlight("Flug sweet", ffmInternational, munichBS, planeUschi);
    Flight londonBerlin = flightService
        .createFlight("Flug Kamikaze", londonCasual, berlinVIPs, planeExcelsior);
    Flight londonMunich = flightService
        .createFlight("Flug", londonRoyal, munichBS, planeEnterprise);
    ...
    // Create Bookings
    Booking bookingFelixLondon = bookingService
        .bookFlight(personFelix, berlinLondon, Lists.newArrayList());
    Booking bookingLenaMunich = bookingService
        .bookFlight(personLena, ffmMunich, Lists.newArrayList());
    Booking bookingNiklasMunich = bookingService
        .bookFlight(personNiklas, ffmMunich, Lists.newArrayList());
    Booking bookingLeonieBerlin = bookingService
        .bookFlight(personLeonie, londonBerlin, Lists.newArrayList());
    Booking bookingLukasMunich = bookingService
        .bookFlight(personLukas, londonMunich, Lists.newArrayList());
  }
}
